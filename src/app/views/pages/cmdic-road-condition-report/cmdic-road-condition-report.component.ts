...
@Component({
	...
	templateUrl: './cmdic-road-condition-report.component.html',
	styleUrls: ['./cmdic-road-condition-report.component.scss'],
})
export class CmdicRoadConditionReportComponent implements OnInit {
	...
	loadSurvey() {
		this.authService.getCurrentReports().subscribe(
			reports => {
				...
				let selectedReport = reportsBySociety[0].reports.filter(report => report.idReport == environment.id_reports.cmdic_road_report);
